# step12: trains/evaluates XGBoost on the step11 radiomics features. Starts
# from the tuned hyperparameters already locked in for the Betti-vector
# XGBoost model (step4_tuning.py / step9c_final_eval_fixed.py) and re-checks
# them against validation AUC on this very different feature space (~100
# named radiomics features vs. 800 anonymous Betti dimensions), using the
# same one-at-a-time sweep-around-a-reference-point approach as step4 -
# only re-tuning where a sweep actually beats the reused baseline.
...
BETTI_XGB_PARAMS = dict(n_estimators=200, max_depth=15, learning_rate=0.3,
                         subsample=0.6, colsample_bytree=0.8)


def load_split(split):
    df = pd.read_csv(os.path.join(base, f"radiomics_features_{split}.csv"))
...
def make_xgb(**overrides):
    params = {**BETTI_XGB_PARAMS, **overrides}
...
def sweep_and_pick(param_name, values, base_params, X_train, y_train, X_val, y_val):
    """Same style as step4_tuning.py: vary one hyperparameter around the reused
    Betti config, holding the others fixed, and report AUC for each value."""
...
if __name__ == "__main__":
    ...
    print(f"Loaded {len(feature_names)} radiomics features. "
    ...
    baseline_auc = val_auc(BETTI_XGB_PARAMS, X_train_s, y_train, X_val_s, y_val)
    print(f"\n=== Reused Betti-vector XGBoost hyperparameters on radiomics features ===")
    print(f"{BETTI_XGB_PARAMS}")
    ...
    for name, values in sweeps:
        best_v, best_auc = sweep_and_pick(name, values, BETTI_XGB_PARAMS, X_train_s, y_train, X_val_s, y_val)
        best_per_param[name] = best_v
        if best_v != BETTI_XGB_PARAMS[name]:
            tuned[name] = best_v
    ...
    if tuned_auc > baseline_auc + 1e-4:
        ...
        final_params = BETTI_XGB_PARAMS
        print("Retuning did not improve on the reused Betti hyperparameters - keeping them as-is.")
    ...
    print("\n=== Test-set results (radiomics features, XGBoost) ===")
    for k, v in m.items():
        print(f"{k}: {v:.4f}")
